...
export function createContentPublisher({
  ...
  return async function publishContent({ wizard, r2Url }) {
    ...
    if (r2Url) {
      try {
        ...
        fileIds = [await pollUploadFileId(upload.id)];
      } catch (err) {
        // Загрузка общая на все аккаунты — если она не удалась, ни одна
        // публикация всё равно не сможет прикрепить файл, поэтому сразу
        // возвращаем отчёт об ошибке по каждому аккаунту, не пытаясь
        // публиковать без файла (для image/video это была бы уже другая,
